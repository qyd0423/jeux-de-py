...
class Snake:
    ...
    def respawn(self, normalBeanss=[], snakeBeanss=[], snakes=[]):
        ...
        self.velCount = 0
    def isDead(self):
        if self.headPos[0] > SCREEN_WIDTH-GRID_SIZE or self.headPos[0] < 0 or self.headPos[1] > SCREEN_HEIGHT-GRID_SIZE or self.headPos[1] < GRID_SIZE:
            return True
        return False
    def getLen(self):
...
def main():
    ...
    while True:
        ...
        # 显示分数
        scoreSurf = scoreFont.render('P1 Score:{}    vs    P2 Score:{}'.format(snake1.getLen()-SNAKE_INIT_LEN, snake2.getLen()-SNAKE_INIT_LEN), True, WHITE)
